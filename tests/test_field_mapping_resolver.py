...
class FieldMappingResolverTests(unittest.TestCase):
    def test_resolve_success_when_price_types_are_same(self) -> None:
        config = build_config()
        ...
        self.assertIn("meal_record", result)

    def test_resolve_raise_when_price_types_mismatch(self) -> None:
        config = build_config()
        bitable = _FakeBitable(
            {
                "tbl_user": [
